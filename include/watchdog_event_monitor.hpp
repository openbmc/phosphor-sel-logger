    ...
    startWatchdogEventMonitor(std::shared_ptr<sdbusplus::asio::connection> conn)
{
    auto watchdogEventMatcherCallback = [conn](
        ...
        boost::container::flat_map<
            std::string, std::variant<std::string, uint64_t, bool>>::iterator
            getWatchdogEnabled,
            getExpireAction, getPreTimeoutInterrupt, getCurrentTimerUse,
            getWatchdogInterval;

        getWatchdogEnabled = watchdogStatus.find("Enabled");
        ...
        getExpireAction = watchdogStatus.find("ExpireAction");
        ...
        getPreTimeoutInterrupt = watchdogStatus.find("PreTimeoutInterrupt");
        std::string_view preTimeoutInterrupt;
        if (getPreTimeoutInterrupt != watchdogStatus.end())
        {
            preTimeoutInterrupt =
                *std::get_if<std::string>(&getPreTimeoutInterrupt->second);
        ...
        {
            eventData[1] &=
                (static_cast<uint8_t>(WatchdogInterruptTypeOffsets::none) << interruptTypeBits);
        }
        else if (preTimeoutInterrupt == "SMI")
        {
            eventData[1] &=
                (static_cast<uint8_t>(WatchdogInterruptTypeOffsets::SMI) << interruptTypeBits);
        }
        else if (preTimeoutInterrupt == "NMI")
        {
            eventData[1] &=
                (static_cast<uint8_t>(WatchdogInterruptTypeOffsets::NMI) << interruptTypeBits);
        }
        ...
        getCurrentTimerUse = watchdogStatus.find("CurrentTimerUse");
        std::string_view currentTimerUse;
        if (getCurrentTimerUse != watchdogStatus.end())
        {
            currentTimerUse =
                *std::get_if<std::string>(&getCurrentTimerUse->second);
        ...
        getWatchdogInterval = watchdogStatus.find("Interval");
        uint64_t watchdogInterval;
        if (getWatchdogInterval != watchdogStatus.end())
        {
            watchdogInterval =
                *std::get_if<uint64_t>(&getWatchdogInterval->second);
        }
        ...
        // TODO: need confirm what's redfishMessageID, the current naming by
        // sensor type
