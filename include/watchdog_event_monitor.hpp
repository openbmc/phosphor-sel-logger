    ...
    startWatchdogEventMonitor(std::shared_ptr<sdbusplus::asio::connection> conn)
{
    auto watchdogEventMatcherCallback = [conn](
        ...
        {
            eventData[1] &=
                (static_cast<uint8_t>(WatchdogInterruptTypeOffsets::none) << 4);
        }
        else if (preTimeoutInterrupt == "SMI")
        {
            eventData[1] &=
                (static_cast<uint8_t>(WatchdogInterruptTypeOffsets::SMI) << 4);
        }
        else if (preTimeoutInterrupt == "NMI")
        {
            eventData[1] &=
                (static_cast<uint8_t>(WatchdogInterruptTypeOffsets::NMI) << 4);
        }
